...
def decode_input(inp, args):    
    ...
    while i < len(inp):        
        item = ''.join(str(int(x)) for x in inp[i:i+len_var])
        indx = int(item,2)        
        if indx >= len(Candidate):            
            indx = len(Candidate) -1
        seed.append(Candidate[indx])  
        
        i += len_var
        
    return list(set(seed))

# convert from pso to bpso
def convert_to_binary(x):
    binx = (np.random.uniform(0.0,1.0, np.shape(x)) >= sigmoid(x))*1.0   
    return binx

# convert decimal to binary
def decimal_to_binary(x,lenn): 
    lenn += 2
    return  format(x, '#0'+str(lenn)+'b')[2:]
